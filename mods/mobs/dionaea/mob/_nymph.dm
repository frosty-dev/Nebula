...
/mob/living/carbon/alien/diona/examine(mob/user)
	. = ..()
	if(holding_item)
		to_chat(user, SPAN_NOTICE("It is holding [icon2html(holding_item)] \a [holding_item]."))
	var/datum/extension/hattable/hattable = get_extension(src, /datum/extension/hattable)
	if(hattable?.hat)
		to_chat(user, SPAN_NOTICE("It is wearing [icon2html(hattable.hat)] \a [hattable.hat]."))
