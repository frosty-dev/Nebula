...
/obj/machinery/shieldgen/interface_interact(mob/user)
	...
	if (src.active)
		user.visible_message("<span class='notice'>[icon2html(src, user)] [user] deactivated the shield generator.</span>", \
			"<span class='notice'>[icon2html(src, user)] You deactivate the shield generator.</span>", \
			"You hear heavy droning fade out.")
		src.shields_down()
	else
		if(anchored)
			user.visible_message("<span class='notice'>[icon2html(src, user)] [user] activated the shield generator.</span>", \
