...
/obj/item/anodevice/Process()
	if(!activated || !inserted_battery || !inserted_battery.battery_effect || !inserted_battery.stored_charge)
		visible_message("<span class='notice'>[icon2html(src, viewers(get_turf(src)))] [src] buzzes.</span>", "<span class='notice'>[icon2html(src, viewers(get_turf(src)))] You hear something buzz.</span>")
		shutdown_emission()
		return
	...
	if(current_tick >= duration)
		visible_message("<span class='notice'>[icon2html(src, viewers(get_turf(src)))] [src] chimes.</span>", "<span class='notice'>[icon2html(src, viewers(get_turf(src)))] You hear something chime.</span>")
		shutdown_emission()
...
/obj/item/anodevice/OnTopic(user, href_list)
	...
	if(href_list["duration"])
		...
		if(inserted_battery && inserted_battery.battery_effect && (inserted_battery.stored_charge > 0) )
			visible_message("<span class='notice'>[icon2html(src, user)] [src] whirrs.</span>", "<span class='notice'>[icon2html(src, user)] You hear something whirr.</span>")
			start_emission()
			. = TOPIC_REFRESH
