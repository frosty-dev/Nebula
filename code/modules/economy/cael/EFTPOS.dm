...
/obj/item/eftpos/attackby(obj/item/O, user)
	...
	if(I)
		if(linked_account)
			...
			to_chat(usr, "[icon2html(src, user)]<span class='warning'>Unable to connect to linked account.</span>")
			...
			if(transaction_locked && !transaction_paid)
				if(!E.is_locked() && transaction_amount <= E.loaded_worth)
					...
					if(linked_account.deposit(transaction_amount, purpose, machine_id))
						...
						visible_message("[icon2html(src, user)] \The [src] chimes.")
						transaction_paid = 1
					else
						to_chat(usr, "[icon2html(src, user)]<span class='warning'>Transaction failed! Please try again.</span>")
				else
					to_chat(usr, "[icon2html(src, user)]<span class='warning'>\The [O] doesn't have that much money!</span>")
		else
			to_chat(usr, "[icon2html(src, user)]<span class='warning'>EFTPOS is not connected to an account.</span>")
